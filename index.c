...
int index_save(const Index *index) {
    // Use malloc — Index is too large (10,000 entries) to copy onto the stack
...
int index_add(Index *index, const char *path) {
    ...
    char *content = malloc((size_t)file_size);
    if (!content) { fclose(f); return -1; }
    if (fread(content, 1, (size_t)file_size, f) != (size_t)file_size) {
        free(content); fclose(f); return -1;
    }
    fclose(f);

    // Store as blob, get its hash
    ObjectID blob_id;
