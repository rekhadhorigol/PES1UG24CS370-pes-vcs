...
static int write_tree_level(IndexEntry *entries, int count, const char *prefix, ObjectID *id_out) {
    ...
    int i = 0;
    while (i < count) {
        // Strip the prefix to get the path relative to this directory level
        const char *rel = entries[i].path + strlen(prefix);

        char *slash = strchr(rel, '/');

        if (slash == NULL) {
            // Plain file at this level — add directly as a blob entry
            ...
            // Subdirectory — group all entries sharing this directory name
            ...
            // Count entries belonging to this subdirectory
            int sub_count = 0;
            int j = i;
