...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    ...
    // Build header like "blob 16" — no null byte yet
    char header[64];
    ...
    // Step 2: Copy everything into one buffer
    ...
    // Step 4: Deduplication — if it exists already, we're done
    if (object_exists(id_out)) {
        free(full);
        return 0;
    }

    // Step 5: Create shard directory e.g. .pes/objects/d5/
    char hex[HASH_HEX_SIZE + 1];
    ...
    // Step 6: Write to a temp file (safe in case of crash)
    char tmp_path[512];
    ...
    // Step 7: fsync — make sure data hits disk before rename
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    // Step 3: Integrity check — rehash and compare
    ObjectID actual_id;
    ...
    char *null_pos = memchr(buf, '\0', (size_t)file_size);
    if (!null_pos) { free(buf); return -1; }

    // Step 5: Parse the type
